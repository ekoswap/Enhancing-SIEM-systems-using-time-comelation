...
for file_name in files:
    ...
    tcp_duration_df = mark_related(
        flows_df[(flows_df["protocol"] == 6) & (flows_df["duration"] > 0)].copy()
    )
    ...
    tcp_duration_total, tcp_duration_pct = summarize(tcp_duration_df)
    ...
    results.append({
        ...
        "tcp_duration_total": tcp_duration_total,
        "tcp_duration_redteam_pct": tcp_duration_pct,
    })

results_df = pd.DataFrame(results)
print(results_df)

output_path = Path("outputs/flows_duration_only_vs_tcp_duration.csv")
